...
st.markdown('<style>divd.block-container{padding-top:1rem;}</style>',unsafe_allow_html= True)


f1 = st.file_uploader(":file_folder: Upload a file",type=(['csv','txt','xslx','xls']))
...
if f1 is not None:
    ...
    os.chdir(r"E:\Project\Streamlit")   
...
with col1:
    ...
    fig = px.bar(category_df, x= 'Category',y='Sales',text = ['${:,.2f}'.format(x) for x in category_df['Sales']],
                 template = "seaborn")
    st.plotly_chart(fig,use_contiainer_width = True,height= 200)
...
with cl1:
    with st.expander('Category_ViewData'):
        ...
        st.download_button("Download Date ",data = csv,file_name = "Category.csv",mime='text/csv',
...
with cl2:
    with st.expander('Region_ViewData'):
        ...
        st.download_button("Download Date ",data = csv,file_name = "region.csv",mime='text/csv',
                           help = 'Click here to download the data as  a CSV file')    
...
# create a tree based on Region, catefory ,sub-category 
